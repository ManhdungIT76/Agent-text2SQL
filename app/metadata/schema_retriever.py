...
class SmartSchemaRetriever:
    ...
    def get_schema_context_from_seed_tables_detailed(self, seed_tables: List[str], max_tables: int = 7):
        """
        Dùng danh sách Bảng Hạt Giống (Seed Tables) từ Prompt 1 (text2sql-table-selector),
        sau đó dùng GraphDB NetworkX tự động nối các Bảng Trung Gian (Junction Tables)
        và trả về (schema_context, seed_tables, all_relevant_names).
        """
        self._initialize_engines()
        tables = self.cache_data.get("tables", {})
        clean_seeds = [str(s).strip().lower() for s in seed_tables if str(s).strip().lower() in tables]
        if not clean_seeds:
            clean_seeds = ["film"]
